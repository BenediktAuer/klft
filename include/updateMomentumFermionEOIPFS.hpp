...
class UpdateMomentumWilsonEOIPFS : public UpdateMomentum {
 ...
 public:
  ...
  UpdateMomentumWilsonEOIPFS() = delete;
  ~UpdateMomentumWilsonEOIPFS() = default;

  UpdateMomentumWilsonEOIPFS(FermionField& phi_,
  ...
  KOKKOS_FORCEINLINE_FUNCTION void operator()(TagEvenContribution,
    ...
    for (size_t mu = 0; mu < rank; ++mu) {
      ...
      momentum(idx, mu) -= 2 * eps * this->factor *
  ...
  void update(const real_t step_size) override {
    ...
    // save \phi^dagger*S_e^-1*S_e^-1*\phi for scaling
    auto dps = spinor_dot_product<rank, Nc, RepDim>(this->chi, this->phi,
                                                    this->dot_product_per_site)
                   .real();
    this->factor = d / dps;
